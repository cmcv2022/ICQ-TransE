...
class LXRTEncoder(nn.Module):
    ...
    def forward(self, sents, feats, visual_attention_mask=None):
        ...
        if args.cap:
            input_ids_cap=sents[4]
            input_mask_cap=sents[5]
            segment_ids_cap=sents[6]

            output = self.model(input_ids, segment_ids, input_mask,qid,input_ids_cap,segment_ids_cap,input_mask_cap,
                                visual_feats=feats,
                                visual_attention_mask=visual_attention_mask)
        else:
            output = self.model(input_ids, segment_ids, input_mask, qid,
                                visual_feats=feats,
                                visual_attention_mask=visual_attention_mask)
        return output
