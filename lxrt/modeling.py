...
class LXRTEncoder(nn.Module):
    ...
    if args.cap:
        def forward(self, lang_feats, lang_attention_mask, qid, caption, cap_mask,
                    visn_feats, visn_attention_mask=None):
            # Run visual embedding layer
            # Note: Word embedding layer was executed outside this module.
            #       Keep this design to allow loading BERT weights.
            if args.glo_fea is True:
                visn_feats0 = self.visn_fc(visn_feats[0:2])
            else:
                visn_feats0 = self.visn_fc(visn_feats)

            if args.glo_fea is True:
                visn_feats0 = torch.cat((visn_feats0.cuda(), visn_feats[2].unsqueeze(1).to(torch.float32)), 1)
                caption0 = caption
            else:
                visn_feats0 = visn_feats0

                # Run relational layers
            for layer_module in self.r_layers:
                visn_feats0 = layer_module(visn_feats0, visn_attention_mask)

            if args.cat is False:
                # Run cross-modality layers
                for layer_module in self.x_layers:
                    caption0, visn_feats0 = layer_module(caption0, cap_mask,
                                                         visn_feats0, visn_attention_mask)

            if args.glo_fea is True:
                if args.cat is False:
                    caption0 = caption+caption0
                else:
                    caption0 = caption
                cap_len = caption0.shape[1]
                lang_attention_mask0 = torch.tensor(
                    np.zeros((lang_attention_mask.shape[0], 1, 1, 1 + caption0.shape[1]))).to(torch.float32)
                lang_attention_mask = torch.cat((lang_attention_mask, lang_attention_mask0.cuda()), 3)

                lang_feats = torch.cat((lang_feats.cuda(), caption0.to(torch.float32)), 1)
                lang_feats = torch.cat((lang_feats.cuda(), visn_feats[3].unsqueeze(1).to(torch.float32)), 1)
            ...
            # Run cross-modality layers
            for layer_module in self.x_layers:
                lang_feats, visn_feats0 = layer_module(lang_feats, lang_attention_mask,
                                                       visn_feats0, visn_attention_mask)

            if args.glo_fea is True:
                d = lang_feats.shape
                d = d[1]
                ll = lang_feats[:, 0:d - 1 - cap_len, :]
                vv = visn_feats0[:, 0:36, :]
            else:
                ll = lang_feats
                vv = visn_feats0
        ...
        def forward(self, lang_feats, lang_attention_mask,qid,
                    visn_feats, visn_attention_mask=None):
            # Run visual embedding layer
            # Note: Word embedding layer was executed outside this module.
            #       Keep this design to allow loading BERT weights.
            ...
            if args.glo_fea is True:
                lang_attention_mask0 = torch.tensor(np.zeros((lang_attention_mask.shape[0], 1, 1, 1))).to(torch.float32)
                lang_attention_mask = torch.cat((lang_attention_mask, lang_attention_mask0.cuda()), 3)
        ...
        try:
            ...
            if pretrained_model_name_or_path == 'bert-base-uncased':
                try:
                    ...
                    archive_file = "../../data/bert/bert-base-uncased.tar.gz"
                    resolved_archive_file = cached_path(archive_file, cache_dir=cache_dir)
                except EnvironmentError:
...
class LXRTModel(BertPreTrainedModel):
    ...
    if args.cap:
        def forward(self, input_ids, token_type_ids=None, attention_mask=None,qid=None,input_ids_cap=None, token_type_ids_cap=None, attention_mask_cap=None,
                ...
                cap_mask=attention_mask_cap.unsqueeze(1).unsqueeze(2)
                ...
                cap_mask=cap_mask.to(dtype=torch.float32)
                cap_mask= (1.0 - cap_mask) * -10000.0
                ...
                embedding_output_cap = self.embeddings(input_ids_cap, token_type_ids_cap)

                # Run LXRT backbone
                lang_feats, visn_feats, qid = self.encoder(
                    embedding_output,
                    extended_attention_mask,
                    qid,
                    embedding_output_cap,
                    cap_mask,
                ...
                return (lang_feats, visn_feats), pooled_output, qid

    else:
        def forward(self, input_ids, token_type_ids=None, attention_mask=None, qid=None, visual_feats=None, visual_attention_mask=None):
            if attention_mask is None:
                attention_mask = torch.ones_like(input_ids)
            if token_type_ids is None:
                token_type_ids = torch.zeros_like(input_ids)
...
class LXRTFeatureExtraction(BertPreTrainedModel):
    ...
    def __init__(self, config, mode='lxr'):
        ...
        self.apply(self.init_bert_weights)
    if args.cap:
        def forward(self, input_ids, token_type_ids=None, attention_mask=None, qid=None, input_ids_cap=None,token_type_ids_cap=None, attention_mask_cap=None, visual_feats=None,
                    visual_attention_mask=None):
            feat_seq, pooled_output,qid = self.bert(input_ids, token_type_ids, attention_mask,qid,input_ids_cap, token_type_ids_cap, attention_mask_cap,
                                                visual_feats=visual_feats,
                                                visual_attention_mask=visual_attention_mask)
            if 'x' == self.mode:
                return feat_seq, pooled_output, qid
            elif 'x' in self.mode and ('l' in self.mode or 'r' in self.mode):
                return feat_seq, pooled_output
            elif 'l' in self.mode or 'r' in self.mode:
                return feat_seq
    else:
        def forward(self, input_ids, token_type_ids=None, attention_mask=None, qid=None,visual_feats=None,
                    visual_attention_mask=None):
            feat_seq, pooled_output, qid = self.bert(input_ids, token_type_ids, attention_mask, qid,
                                                     visual_feats=visual_feats,
                                                     visual_attention_mask=visual_attention_mask)
            if 'x' == self.mode:
                return feat_seq,pooled_output,qid
